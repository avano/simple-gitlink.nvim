...
-- execute the function in the parent directory of the currently opened file and switch back after
local function with_cwd(fn)
  local prev_dir = vim.loop.cwd()
  local file_dir = vim.fn.expand("%:p:h")
  debug('Changing directory to ' .. file_dir)
  vim.loop.chdir(file_dir)
  local ok, result = pcall(fn)
  debug('Changing directory back to ' .. prev_dir)
  vim.loop.chdir(prev_dir)
...
function M.create_git_link()
  with_cwd(function()
    ...
    local range = start_line == end_line and '#L' .. start_line or
    string.format('#L%d-L%d', math.min(start_line, end_line), math.max(start_line, end_line))
